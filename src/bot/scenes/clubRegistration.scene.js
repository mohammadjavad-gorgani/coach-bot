...
const { clubValidationSchema } = require("../../validations/clubRegistration.validation");
...
const clubRegistration = new Scenes.WizardScene(
    ...
    async (ctx) => {
        ...
        const { error } = clubValidationSchema.extract("club_name").validate(club_name);
    ...
    async (ctx) => {
        ...
        const { error } = clubValidationSchema.extract("club_address").validate(club_address);
    ...
    async (ctx) => {
        ...
        const { error } = clubValidationSchema.extract("club_phone").validate(club_phone);
        ...
        try {
            ...
            await ctx.telegram.sendMessage(ctx.chat.id, `${user.full_name} ${mainMenuMessages.mainText}`, mainMenu());
        } catch (error) {
